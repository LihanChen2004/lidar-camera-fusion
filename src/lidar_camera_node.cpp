...
void callback(const boost::shared_ptr<const sensor_msgs::PointCloud2> &in_pc2,
  ...
  try {
    ...
    color_pcl =
        cv_bridge::toCvCopy(in_image, sensor_msgs::image_encodings::BGR8);
  ...
  rangeImage->pcl::RangeImage::createFromPointCloud(
      *cloud_out, pcl::deg2rad(angular_resolution_x),
      pcl::deg2rad(angular_resolution_y), pcl::deg2rad(max_angle_width),
      pcl::deg2rad(max_angle_height), sensorPose, coordinate_frame, 0.0f, 0.0f,
      0);
      ...
      if (!(Zout(i, j) == 0)) {
        ...
        Eigen::MatrixXf Lidar_matrix(
            3, 3); // matrix  transformation between lidar and range image. It
                   // rotates the angles that it has of error with respect to the
                   // ground
        Eigen::MatrixXf result(3, 1);
        Lidar_matrix << cos(ang_x_lidar), 0, sin(ang_x_lidar), 0, 1, 0,
  ...
  RTlc << Rlc(0), Rlc(3), Rlc(6), Tlc(0), Rlc(1), Rlc(4), Rlc(7), Tlc(1),
      Rlc(2), Rlc(5), Rlc(8), Tlc(2), 0, 0, 0, 1;
  ...
  for (int i = 0; i < size_inter_Lidar; i++) {
    ...
    cv::circle(cv_ptr->image, cv::Point(px_data, py_data), 1,
               CV_RGB(255 - color_dis_x, (int)(color_dis_z), color_dis_x),
               cv::FILLED);
  }
...
int main(int argc, char **argv) {
  ...
  Rlc << (double)param[0], (double)param[1], (double)param[2], (double)param[3],
  ...
  rangeImage =
      boost::shared_ptr<pcl::RangeImageSpherical>(new pcl::RangeImageSpherical);
